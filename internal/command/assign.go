	...
	for i := startPad; i < prog.PadCount() && si < len(samples); i++ {
		...
		for j := 0; j < 4 && si < len(samples); j++ {
			if matrix.Get(i, j) == nil {
				...
				pad.Layer(j).SetSampleName(sample.Name)
...
func MultisampleAssign(prog *pgm.Program, matrix *pgm.SampleMatrix, samples []*pgm.SampleRef) ([]int, []string) {
	...
	for i, slot := range slots {
		...
		layer.SetSampleName(slot.Source.Name)
	...
	return modified, builder.Warnings
}
