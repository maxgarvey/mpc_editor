...
func main() {
	...
	files := []struct {
...

		{
			...
			seq.Create(120.0, 1, "Track01", "", false, []seq.Event{
...

		{
			...
			seq.Create(120.0, 1, "Track01", "", false, func() []seq.Event {
...

		{
			...
			seq.Create(120.0, 2, "Track01", "", false, func() []seq.Event {
				evs := []seq.Event{}
...

		{
			...
			seq.Create(90.0, 1, "Track01", "", false, []seq.Event{
				{Tick: 0, Track: 0, Type: seq.EventNoteOn, Note: 36, Velocity: 100, Duration: 23},
				{Tick: 96, Track: 0, Type: seq.EventNoteOn, Note: 36, Velocity: 100, Duration: 23},
				{Tick: 192, Track: 0, Type: seq.EventNoteOn, Note: 36, Velocity: 100, Duration: 23},
				{Tick: 288, Track: 0, Type: seq.EventNoteOn, Note: 36, Velocity: 100, Duration: 23},
...

		{
			...
			seq.Create(120.0, 4, "Track01", "", false, func() []seq.Event {
				evs := []seq.Event{}
				for bar := 0; bar < 4; bar++ {
		...
		// ── Loop flag verification (load on hardware; check Loop setting) ──

		{
			"verify_loop_off.SEQ",
			// Identical to verify_loop_on.SEQ except loop=false (byte 0x17=0x00).
			// Expected on hardware: Loop = OFF
			seq.Create(120.0, 1, "Track01", "", false, []seq.Event{
				{Tick: 0, Track: 0, Type: seq.EventNoteOn, Note: 36, Velocity: 100, Duration: 23},
			}),
		},
		...
		{
			"verify_loop_on.SEQ",
			// Identical to verify_loop_off.SEQ except loop=true (byte 0x17=0x01).
			// Expected on hardware: Loop = 1-End (or equivalent looping mode)
			seq.Create(120.0, 1, "Track01", "", true, []seq.Event{
				{Tick: 0, Track: 0, Type: seq.EventNoteOn, Note: 36, Velocity: 100, Duration: 23},
			}),
		},
	}
